...
You've probably realised the answer is it depends. If you enter 10 like we did before the number of steps is 33 but if you enter another number then the number of steps changes. There is no concrete number we can use to measure the efficiency of our code because it changes based on an external input.
...
We can still access any element in just one step. `arr[7]` gives me `8` in a single step. Our array can keep growing and we can always access any element in a single step. It's constant. Hence we have `O(1)`.
...
One such algorithm that does this is Binary Search. It only works on sorted arrays but if you have an array of 10 items in sorted order and wanted to know if it had the number `7`, Binary Search would guess the middle item of the array and see what is there. Because the array is sorted, if the number at the middle index was `5`, then we know anything to the left of that index cannot be the number 7, as those items must be lower than 5 in a sorted array. Therefore in just one step we've eliminated half of the array. We can do the same with the remaining half. We can guess the middle index and see if it's 7. Half of that (half of an array) array eliminated again.
...
If we write two algorithms with the same complexity. Does that mean they're equally good to use? I'll answer this question with two code examples which we'll then discuss a bit further to try and answer the question
...
1.  Read through [A Rubyists Guide to Big-O Notation](https://www.honeybadger.io/blog/a-rubyist-s-guide-to-big-o-notation/)]. The last section talks about how this applies to database queries in a web app but which you won't have covered yet but it's something to keep in mind as you move forward into the Rails materials
